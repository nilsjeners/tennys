...
export default function TestVPage() {
  ...
  const handleScroll = (deltaY: number) => {
    ...
    const nextWinner: Winner = deltaY < 0 ? "A" : "B";
  ...
  const handleSaveSet = () => {
    if (sets.length >= MAX_SETS || winner === null) {
      return;
    }
  ...
  return (
    <main className="min-h-screen">
      <div
        className="relative grid min-h-screen w-full grid-rows-2 overscroll-none touch-none"
        onWheel={handleWheel}
      ...
      >
        <section
          className="flex items-center justify-center bg-gradient-to-br from-[#0c1220] to-[#111c31] text-base-100"
          onClick={() => {
            setWinner("A");
            setScoreIndex(0);
          }}
        >
          <div className="flex flex-col items-center gap-2 uppercase tracking-[0.12em]">
            ...
            <span className="text-xs text-base-100/70">Wisch nach oben</span>
          </div>
        </section>
        <section
          className="flex items-center justify-center bg-gradient-to-tr from-[#f97316] to-[#111c31] text-base-100"
          onClick={() => {
            setWinner("B");
            setScoreIndex(0);
          }}
        >
          <div className="flex flex-col items-center gap-2 uppercase tracking-[0.12em]">
            ...
            <span className="text-xs text-base-100/70">Wisch nach unten</span>
          </div>
        </section>
        ...
        <div className="pointer-events-none absolute inset-0 flex flex-col items-center justify-center gap-4 px-4">
          ...
          <div className="pointer-events-auto flex flex-wrap justify-center gap-3">
            <button
              ...
              disabled={sets.length >= MAX_SETS || winner === null}
            >
              Satz speichern
            </button>
            <button
